/**
 * Claude Analysis Provider Tests
 * Comprehensive unit tests for Claude 3.5 Sonnet functionality
 */
...
// Mock environment
const mockEnv = {
  ANTHROPIC_API_KEY: 'test-claude-key-456'
};
...
describe('ClaudeAnalysisProvider', () => {
  ...
  beforeEach(async () => {
    const module = await import('../../providers/claude-analysis.ts');
    provider = new module.ClaudeAnalysisProvider();
  });

  describe('constructor', () => {
    it('should initialize with correct name and model', () => {
      assertEquals(provider.name, 'claude-analysis');
    });

    it('should warn when API key is missing', async () => {
      Deno.env.get = () => '';
      const module = await import('../../providers/claude-analysis.ts');
      const providerWithoutKey = new module.ClaudeAnalysisProvider();
      assertEquals(providerWithoutKey.name, 'claude-analysis');
    });
  });
  ...
  describe('extractTextFromImage', () => {
    ...
    it('should successfully extract text from image', async () => {
      const mockResponse = {
        content: [{
          text: 'Extracted text from document using Claude vision'
        }]
      };
      ...
      assertEquals(result.text, 'Extracted text from document using Claude vision');
    ...
    it('should handle base64 data URL properly', async () => {
      ...
      const imageContent = body.messages[0].content.find((c: any) => c.type === 'image');
      
      assertEquals(imageContent.source.type, 'base64');
      assertEquals(imageContent.source.media_type, 'image/png');
      assertEquals(typeof imageContent.source.data, 'string');
    });
    ...
    it('should make correct API request', async () => {
      ...
      assertEquals(capturedRequest.url, 'https://api.anthropic.com/v1/messages');
      assertEquals(capturedRequest.options.method, 'POST');
      assertEquals(capturedRequest.options.headers['Authorization'], 'Bearer test-claude-key-456');
      assertEquals(capturedRequest.options.headers['anthropic-version'], '2023-06-01');
      
      const body = JSON.parse(capturedRequest.options.body);
      assertEquals(body.model, 'claude-3-5-sonnet-20241022');
  ...
  describe('analyzeDocument', () => {
    ...
    it('should clean JSON from markdown formatting', async () => {
      const mockResponse = {
        content: [{
          text: '```json\n{"document_type": "passport", "confidence": 0.9, "suggested_form": "visa_application", "extracted_data": {}}\n```'
        }]
      };
    ...
    it('should handle JSON extraction from mixed content', async () => {
      const mockResponse = {
        content: [{
          text: 'Here is the analysis:\n\n{"document_type": "financial", "confidence": 0.85, "suggested_form": "financial_declaration", "extracted_data": {"bank_name": "Test Bank"}}\n\nThis document appears to be a bank statement.'
        }]
      };
    ...
    it('should handle large text input by truncating', async () => {
      ...
      globalThis.fetch = (url: string, options?: RequestInit) => {
        capturedRequest = { url, options };
        return Promise.resolve(new Response(JSON.stringify({
          content: [{ text: '{"document_type": "other", "confidence": 0.5, "suggested_form": "personal_information", "extracted_data": {}}' }]
        }), { status: 200 }));
      };
